...
#define MAVLINK_MSG_ID_ACK_LEN 19
#define MAVLINK_MSG_ID_ACK_MIN_LEN 19
#define MAVLINK_MSG_ID_150_LEN 19
#define MAVLINK_MSG_ID_150_MIN_LEN 19

#define MAVLINK_MSG_ID_ACK_CRC 244
#define MAVLINK_MSG_ID_150_CRC 244

#define MAVLINK_MSG_ACK_FIELD_PARAM_LEN 16
...
#define MAVLINK_MESSAGE_INFO_ACK { \
	...
	{  { "err_code", NULL, MAVLINK_TYPE_UINT16_T, 0, 0, offsetof(mavlink_ack_t, err_code) }, \
         ...
         { "param", NULL, MAVLINK_TYPE_UINT8_T, 16, 3, offsetof(mavlink_ack_t, param) }, \
         } \
}
#else
#define MAVLINK_MESSAGE_INFO_ACK { \
...
{
	return mavlink_msg_ack_pack(system_id, component_id, msg, ack->msg_id, ack->err_code, ack->param);
}
...
{
	return mavlink_msg_ack_pack_chan(system_id, component_id, chan, msg, ack->msg_id, ack->err_code, ack->param);
}
...
static inline void mavlink_msg_ack_send(mavlink_channel_t chan, uint8_t msg_id, uint16_t err_code, const uint8_t *param)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[MAVLINK_MSG_ID_ACK_LEN];
	_mav_put_uint16_t(buf, 0, err_code);
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_ack_send(chan, ack->msg_id, ack->err_code, ack->param);
#else
 ...
 */
static inline void mavlink_msg_ack_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint8_t msg_id, uint16_t err_code, const uint8_t *param)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
...
/**
 * @brief Get field param from ack message
 *
 * @return 
 */
static inline uint16_t mavlink_msg_ack_get_param(const mavlink_message_t* msg, uint8_t *param)
{
	return _MAV_RETURN_uint8_t_array(msg, param, 16,  3);
}
...
static inline void mavlink_msg_ack_decode(const mavlink_message_t* msg, mavlink_ack_t* ack)
{
	...
	mavlink_msg_ack_get_param(msg, ack->param);
#else
