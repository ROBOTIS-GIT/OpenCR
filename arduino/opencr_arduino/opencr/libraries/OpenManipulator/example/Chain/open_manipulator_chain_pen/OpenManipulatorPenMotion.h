...
void demo_motion_robotis_mm(OPEN_MANIPULATOR_PEN *open_manipulator, int cnt)
{
  ...
  switch(cnt) 
  {
    case 0: // R start pose 0, 0
      open_manipulator->taskTrajectoryMoveToPresentPose("pen", RM_MATH::makeVector3(0.0150, 0.015, -0.0), 0.3);
    break;
    case 1: // R start pose 0, 0
      open_manipulator->taskTrajectoryMoveToPresentPose("pen", RM_MATH::makeVector3(-0.0150, -0.015, -0.0), 0.3);
    break;
    case 2: // R start pose 0, 0
      open_manipulator->taskTrajectoryMoveToPresentPose("pen", RM_MATH::makeVector3(-0.00, -0.0, -0.0), 3.0);
    break;
    // case 0: // home
...
void demo_motion_mm(OPEN_MANIPULATOR_PEN *open_manipulator, int cnt)
{
  std::vector<double> goal_joint_position;
  char draw_alphabet_arg[1];
  void* p_draw_alphabet_arg = &draw_alphabet_arg;
...
void demo_motion_robotis(OPEN_MANIPULATOR_PEN *open_manipulator, int cnt)
{
  std::vector<double> goal_joint_position;
  char draw_alphabet_arg[1];
  void* p_draw_alphabet_arg = &draw_alphabet_arg;
  ...
  {
    case 0: // home
      goal_joint_position.push_back( 0.940);
      goal_joint_position.push_back(-0.008);
      goal_joint_position.push_back(-0.204);
      goal_joint_position.push_back( 0.140);
      open_manipulator->jointTrajectoryMove(goal_joint_position, 1.0); 
    break;
    // case 1: // init
    //   goal_joint_position.push_back( 0.00);
...
{
  if(digitalRead(BDPIN_PUSH_SW_1) && demo_motion_type != 1)
  {
    demo_motion_state = true;
    demo_motion_type = 1; // turn
  }

  else if(digitalRead(BDPIN_PUSH_SW_2) && demo_motion_type == 1)
  {
    demo_motion_state = true;
    demo_motion_type = 3; // 
  }
  else if(digitalRead(BDPIN_PUSH_SW_2) && demo_motion_state == false)
  {
    demo_motion_state = true;
