...
#include "../include/open_manipulator_libs/Delta_Joint.h"
...
void JointDynamixel::setMode(std::vector<uint8_t> actuator_id, const void *arg)
{
  ...
  if (get_arg_[0] == "position_mode" || get_arg_[0] == "current_based_position_mode" || get_arg_[0] == "extended_position_mode")
  {
...
{
  const char* log = NULL; // <--- why const char*? why not like string...
  bool result = false;
  
  for (uint32_t index = 0; index < dynamixel_.num; index++)
  {
    result = dynamixel_workbench_->torqueOn(dynamixel_.id.at(index), &log); // <---.id.at()???
    if (result == false)
    {
      RM_LOG::ERROR(log);
    }
...
{    // <--- not used???
  ...
  {
    radian_vector.push_back(value_vector.at(index).value); // <--- why only value.. ??? 
  }
...
bool JointDynamixel::initialize(std::vector<uint8_t> actuator_id, STRING dxl_device_name, STRING dxl_baud_rate) // <--- dxl_id
{
  bool result = false;
  const char* log = NULL;
  ...
  dynamixel_workbench_ = new DynamixelWorkbench;  // <--- () not needed?  what should i refer to to know new ~~...
...
bool JointDynamixel::setOperatingMode(std::vector<uint8_t> actuator_id, STRING dynamixel_mode)
{
  ...
  else if (dynamixel_mode == "extended_position_mode")
  {
    for (uint8_t num = 0; num < actuator_id.size(); num++)
    {
      result = dynamixel_workbench_->setExtendedPositionControlMode(actuator_id.at(num), &log); 
      if (result == false)
      {
        RM_LOG::ERROR(log);
      }
...
bool JointDynamixel::setSDKHandler(uint8_t actuator_id)  // <--- where are you guys used..?/
{
  bool result = false;
  const char* log = NULL;
...
bool JointDynamixel::writeProfileValue(std::vector<uint8_t> actuator_id, STRING profile_mode, uint32_t value) // <--- where are you guys used..?/
{
  const char* log = NULL;
  bool result = false;
