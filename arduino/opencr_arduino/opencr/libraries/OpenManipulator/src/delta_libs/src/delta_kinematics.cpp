...
bool SolverUsingGeometry::inverseKinematicsSolverUsingGeometry(Manipulator *manipulator, Name tool_name, Pose target_pose, std::vector<JointValue> *goal_joint_value)
{
  ...
  for (int i=0; i<3; i++){
    target_x[i] = target_pose.kinematic.position(0) + cos(PI*2.0/3.0*i)*(-link[2]);
    target_y[i] = target_pose.kinematic.position(1) + sin(PI*2.0/3.0*i)*(-link[2]);
    target_z[i] = target_pose.kinematic.position(2);
  }
  ...
  for (int i=0; i<3; i++){
    ...
    target_angle[2*i+3].position = PI/2 + acos(-diff_z2[i]/link[1]) - target_angle[i].position;
    ...
    target_angle[2*i+4].position = asin(temp4[i] / sqrt(link[1]*link[1]-diff_z2[i]*diff_z2[i]));
  ...
  target_angle2[0].position = target_angle[4].position;
  target_angle2[1].position = target_angle[6].position;
  target_angle2[2].position = target_angle[8].position;
  ...
  Serial.println(target_angle[0].position,4);
  Serial.println(target_angle[1].position,4);
  Serial.println(target_angle[2].position,4);
  ...
  Serial.println(target_angle2[0].position,4);
  Serial.println(target_angle2[1].position,4);
  Serial.println(target_angle2[2].position,4);
