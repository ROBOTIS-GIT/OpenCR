...
#include "../include/open_manipulator_libs/Planar_Drawing.h"

using namespace Eigen;
using namespace PLANAR_DRAWING;
...
std::vector<WayPoint> Heart::drawHeart(double tick)
{
  ...
  traj[0] =  - 1.0f/17.0f*7
    + (1.0f/17.0f*(13*cos(get_time_var) - 5*cos(2*get_time_var) - 2*cos(3*get_time_var) - cos(4*get_time_var)));
  traj[1] = 1.0f/17.0f*(16*sin(get_time_var)*sin(get_time_var)*sin(get_time_var));
