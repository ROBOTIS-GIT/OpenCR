...
std::vector<float> Planar::inverseKinematics(OM_MANAGER::Manipulator *manipulator, Name tool_name, Pose target_pose)
{
  const float lambda = 0.7;
  const int8_t iteration = 10;
...
std::vector<float> Planar::srInverseKinematics(OM_MANAGER::Manipulator *manipulator, Name tool_name, Pose target_pose)
{
  float lambda = 0.0;
  const float param = 0.002;
  const int8_t iteration = 50;
...
std::vector<float> Planar::positionOnlyInverseKinematics(OM_MANAGER::Manipulator *manipulator, Name tool_name, Pose target_pose)
{
  float lambda = 0.0;
  const float param = 0.002;
  const int8_t iteration = 10;
