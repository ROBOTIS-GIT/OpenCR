...
using namespace ROBOTIS_MANIPULATOR; // <--- better be manager...
...
void Stewart::forwardKinematics(Manipulator *manipulator)  
{
  forwardKinematicsSolverUsingGeometry(manipulator, manipulator->getWorldChildName());
}

bool Stewart::inverseKinematics(Manipulator *manipulator, Name tool_name, Pose target_pose, std::vector<double> *goal_joint_value)
{
...
void Stewart::forwardKinematicsSolverUsingGeometry(Manipulator *manipulator, Name component_name){}

bool Stewart::inverseKinematicsSolverUsingGeometry(Manipulator *manipulator, Name tool_name, Pose target_pose, std::vector<double> *goal_joint_value)
{
  ...
  for (int i=0; i<6; i++){
    if (i%2 == 0) {
      temp_x[i] = target_pose.position(0) + cos(PI*2.0/3.0*(i/2) - 0.911)*(-0.07825);
      temp_y[i] = target_pose.position(1) + sin(PI*2.0/3.0*(i/2) - 0.911)*(-0.07825);
    } 
    else {
      temp_x[i] = target_pose.position(0) + cos(PI*2.0/3.0*(i/2) + 0.911)*(-0.07825);
      temp_y[i] = target_pose.position(1) + sin(PI*2.0/3.0*(i/2) + 0.911)*(-0.07825);
