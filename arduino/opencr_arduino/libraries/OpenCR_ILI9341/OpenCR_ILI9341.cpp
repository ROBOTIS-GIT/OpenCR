...
static uint8_t done_reset, is8347=0;
...
#if defined(ESP8266) || defined (ARDUINO_ARCH_ARC32) || defined(__OPENCR__)
...
void Adafruit_ILI9341::WriteCmdParamN(uint16_t cmd, int8_t N, uint8_t * block)
{
  digitalWrite(_sclk, LOW);
  digitalWrite(_cs, LOW);
...
void Adafruit_ILI9341::WriteCmdData(uint16_t cmd, uint16_t dat)
{
  digitalWrite(_sclk, LOW);
  digitalWrite(_cs, LOW);
 ...
 */
 ...
 void Adafruit_ILI9341::vertScroll(int16_t top, int16_t scrollines, int16_t offset)
 {
