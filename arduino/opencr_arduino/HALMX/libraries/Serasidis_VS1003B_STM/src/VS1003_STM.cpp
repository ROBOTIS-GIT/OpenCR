...
void VS1003::begin(void)
{
  ...
  //write_register(SCI_CLOCKF,0x8800);
  delay(1);
  await_data_request();
...
void VS1003::print_byte_register(uint8_t reg) const
{
  ...
  Serial1.print(name);
  Serial1.print('\t');
  if(extra_tab > 0) Serial1.write(extra_tab);
  Serial1.println(read_register(reg),DEC);
}
...
{
  Serial1.println(("VS1003 Configuration:"));
  int i = 0;
...
/****************************************************************************
 * Enable the VS1053 I2S interface. By Vassilis Serasidis <info@serasidis.gr> 
 ****************************************************************************/

void VS1003::enableI2S(void)
{
  //Set GPIO as outputs
  write_register(SCI_WRAMADDR, 0xc017);
	write_register(SCI_WRAM, 0x00f3); //VS1053
  //write_register(SCI_WRAM, 0x0033); //VS1033
  
  //Enable I2S (MCLK enabled, 48 kHz sample rate)
	write_register(SCI_WRAMADDR, 0xc040);
	write_register(SCI_WRAM, 0x000c);
	
}
/****************************************************************************/

void VS1003::softReset(void){
  delay(100);
	write_register(SCI_MODE, (1<<SM_SDINEW) | (1<<SM_RESET));
	delay(100);
}
